...
/***/ function(module, exports, __webpack_require__) {
	...
	Xa.defineModule("/pm/draft/draft", function () {
	    var deleteItem = {};
	    console.count("draft");
	    function controllerAction(config) {
	        ...
	        config.controller[config.action](config.data).done(function (data) {
	            _this.msg.type = "";
	    ...
	    return {
	        ...
	        watch: {},
	        methods: {
	            handleDeleteAction: function (item) {
	                this.delete.dialogShow = true;
	                this.delete.deleteItem = item;
	                this.delete.action = "deleteItem";
	                deleteItem = item;
	            },
	            handleDialogAction: function (action, id) {
	                this.delete.dialogShow = false;
	                var _this = this;
	                if (action == "确定" && this.delete.action == "deleteItem") {
	                    console.info(JSON.stringify(deleteItem));
	                    var config = {
	                        controller: inboxController,
	                        action: "delete_case",
	                        data: { app_uid: deleteItem.app_uid },
	                        fn: {
	                            done: function (data) {
	                                this.inboxItems.$remove(deleteItem);
	                                this.inboxItem.total.forEach(function (params) {
	                                    console.log(params.app_pro_title);
	                                    if (params.app_pro_title == '全部' || params.app_pro_title == deleteItem.app_pro_title) {
	                                        params.num--;
	                                    }
	                                });
	                            },
	                            fail: function (data) {
	                                console.error("Draft:delete_case#" + JSON.stringify(data));
	                            }
	                        },
	                        msg: {
	                            loading: function () {
	                    ...
	                    controllerAction.call(this, config);
	                }
	            },
	        },
	        events: {
	        ...
	        route: {
	            data: function (transition) {
	                var _this = this;
	                inboxController.draft_list({}).done(function (data) {
	        ...
	        filters: {},
	        ready: function () {
	            this.$root.seoPageInfo.title = "我的草稿";
	        },
	        components: {
