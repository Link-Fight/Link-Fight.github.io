...
/***/ function(module, exports, __webpack_require__) {
	...
	window.Vue = __webpack_require__(300);
	var VueRouter = __webpack_require__(301);
	var views = {};
	var versionsDefferred = null;
	var simulateRouteQuery = {};
	window.$ = __webpack_require__(302);
	...
	window.Xa = {
	    ajax: function ajax(url, data, callback, type) {
	        var reg = /(\.html|\.htm$|\.json|\.txt)$/i;
	        if (reg.test(url)) Xa.get(url, data, callback, type);else Xa.post(url, data, callback, type);
	    },
	    post: function post(url, data, callback, type) {
	        data = data || {};
	        try {
	            if (!type) type = "json";
	            data.fromUrl = window.location.pathname + window.location.hash;
	            $.post(url, data, function (ret) {
	                ...
	                } else if (ret.status == 9999) {
	                    Xa.start("html");
	                    Router.go('/login?goto=' + window.location.hash.substr(2));
	                } else if (ret.status == 4304) {
	                    Xa.start("html");
	                    Router.go('/error?msg=登录验证失败');
	                } else if (ret.status == 4300) {
	                    Xa.start("html");
	                    Router.go('/error?msg=您没有权限查看此页面');
	                } else {
	                    callback(ret);
	                }
	            }).fail(function (xhr) {
	    ...
	    get: function get(url, data, callback, type) {
	        try {
	            ...
	            }, "html").fail(function (xhr, a, b) {
	                /*可以跳到错误页面带当前页面url，错误页面提供刷新功能功能，地址为Router.go(url)*/
	                alert('服务器内部错误');
	            });
	        } catch (e) {}
	        /*
	    ...
	    Array: {
	         ...
	         * @returns {Array}
	         */
	        union: function union(target, array) {
	            return Xa.Array.unique(Xa.Array.merge(target, array));
	        },
	         ...
	         * @returns {Array}
	         */
	        diff: function diff(target, array) {
	    ...
	    Date: {
	        format: function format(date, patten) {
	            var date = date ? date : new Date();
	            if (!patten) {
	                return date.toString();
	            }
	            var list = { "y+": date.getFullYear(), "M+": date.getMonth() + 1, "d+": date.getDate(), "[Hh]+": date.getHours(), "m+": date.getMinutes(), "s+": date.getSeconds(), "S+": date.getMilliseconds() };
	            for (var p in list) {
	                var reg = new RegExp("(" + p + ")");
	                if (reg.test(patten)) {
	                    var str = "00" + list[p];
	                    var len = Math.max(RegExp.$1.length, ("" + list[p]).length);
	                    patten = patten.replace(RegExp.$1, str.substr(str.length - len));
	                }
	            }
	            return patten;
	        },
	        toStrong: function toStrong(date, format) {
	            var date = date ? date : new Date();
	            if (!format) format = "yyyy-MM-dd";
	            switch (typeof date === "undefined" ? "undefined" : _typeof(date)) {
	                case "string":
	                    var reg = /(\d{4})\D+(\d{1,2})\D+(\d{1,2})\s*(\d*)\D*(\d*)\D*(\d*)/;
	                    var year, month, day, hour, min, sec;
	                    date.replace(reg, function ($0, $1, $2, $3, $4, $5, $6) {
	                        year = $1, month = $2, day = $3, hour = $4, min = $5, sec = $6;
	                    });
	                    date = new Date(year, month, day, hour, min, sec); /*FF下要传入六个参数*/
	                    break;
	                case "number":
	                    date = new Date(date);
	                    break;
	            ...
	            return format.replace(/(yyyy|MM?|dd?|HH?|ss?|mm?)/g, function ($0) {
	                return result[$0];
	            });
	        }
	    },
	    DateTime: {
	        ...
	        toStrong: function toStrong(dateTime) {
	            var date = dateTime;
	            date = date.split("T");
	            date = date.join(" ");
	            return date;
	    ...
	    /*params 参数用于支持手动调用requireModule,而且请求的页面需要在地址栏带参数的,参数可以通过params传入*/

	    requireModule: function requireModule(name, params) {
	        simulateRouteQuery = params;
	        if (views[name] && views[name].component) return views[name].component;
	        views[name] = {
	            name: name,
	            component: Vue.component(name, function (resolve) {
	                views[name].resolve = resolve;
	                var path = "./views" + name + ".js";
	                if (versionsDefferred == null) {
	                    versionsDefferred = $.ajax({
	                        url: './version.json',
	                        dataType: "json",
	                        cache: false
	                    });
	                }
	                versionsDefferred.done(function (versions) {
	                    var v = versions[path];
	                    $.ajax({
	                        url: path + (v ? '?v=' + v : ''),
	                        dataType: "script",
	        ...
	        return views[name].component;
	    },
	    hasModule: function hasModule(name) {
	        if (views[name] && views[name].component) {
	            if (views[name].resolve instanceof Function) {
	                return false;
	            }
	            return true;
	        }
	    ...
	    defineModule: function defineModule(name, vueCreater) {
	        /*simulateRouteQuery手动传入页面需要的参数*/
	        views[name].resolve(vueCreater(Vue, simulateRouteQuery));
	    },
	    loadComponent: function loadComponent(name, url, data, vueParams) {
	        return Vue.component(name, function (resolve) {
	            Xa.get(url, data, function (html) {
	                resolve($.extend(vueParams || {}, {
	                    template: html
	                }));
	            });
	        });
	    },
	    start: function start(appElement) {
	        Vue.use(VueRouter);
	        var routerConfig = __webpack_require__(320);
	        var app = Vue.extend({
	            data: function data() {
	                return {
	                    seoPageInfo: {
	            ...
	            watch: {
	                'seoPageInfo.title': function seoPageInfoTitle(val, oldVal) {
	                    var body = document.getElementsByTagName('body')[0];
	                    document.title = val;
	                    var iframe = document.createElement("iframe");
	                    iframe.style.display = 'none'; //隐藏，否则会闪现一个框框
	                    iframe.setAttribute("src", "images/loading_data.gif");
	        ...
	        Router.alias(routerConfig.alias);
	        Router.start(app, appElement);
	    },
...
/***/ function(module, exports, __webpack_require__) {
	...
	// module
	exports.push([module.id, "html,body,#app,section{height:100%;}\r\nbody{background:#F2F2F2;font-size:14px;color:#333;}\r\ntable,p{font-size:14px;}\r\nul,li{margin:0;padding:0;list-style:none;}\r\ninput{padding:0;}\r\ntextarea{padding:0;}\r\n.fr{float:right;}\r\n.fl{float:left;}\r\n.clearfix {zoom: 1;}\r\n.clearfix:after {content:\"\";display: block;clear: both;height: 0;}\r\n\r\n/*重置weui样式*/\r\n.weui_cells_form .weui_cell_hd{padding:0}\r\n.weui_label{width:5.5em;}\r\n.weui_cells{font-size:14px;color:#555}\r\n.weui_panel{color:#555}\r\n/*无右括号*/\r\n.weui_cell_text_ft{text-align: right;color:#c8c8cd;}\r\n/*标题带子目录*/\r\n.weui_panel_hd_cell{\r\n    position: relative;\r\n    display: -webkit-box;\r\n    display: -webkit-flex;\r\n    display: -ms-flexbox;\r\n    display: flex;\r\n    -webkit-box-align: center;\r\n    -webkit-align-items: center;\r\n    -ms-flex-align: center;\r\n    align-items: center;\r\n    color:#666;\r\n}\r\n.weui_panel_hd_primary{\r\n    -webkit-box-flex: 1;\r\n    -webkit-flex: 1;\r\n    -ms-flex: 1;\r\n    flex: 1;\r\n}\r\n\r\n\r\n/*全局颜色设置*/\r\n.global-bg-red{\r\n    background-color: #f00;\r\n}\r\n.global-bg-blue{\r\n    background-color: #3896C9;\r\n}\r\n.global-bg-yellow{\r\n    background-color: #ffcc00;\r\n}\r\n.global-bg-yellow1{\r\n    background-color: #FDFFCB;\r\n}\r\n.global-color-green{\r\n    color:#3cc51f;\r\n}\r\n.global-color-gray{\r\n    color:#888;\r\n}\r\n.global-color-blue{\r\n    color:#55bafd;\r\n}\r\n.global-color-yellow{\r\n    color:#fd9801;\r\n}\r\n.global-color-red{\r\n    color:#f00;\r\n}\r\n\r\n/*遮罩层*/\r\n.global-mask{margin-bottom: 10px;}\r\n.global_mask_transition {\r\n    display: none;\r\n    position: fixed;\r\n    z-index: 1;\r\n    width: 100%;\r\n    height: 100%;\r\n    top: 0;\r\n    left: 0;\r\n    background: rgba(0, 0, 0, 0);\r\n    -webkit-transition: background .3s;\r\n    transition: background .3s;\r\n}\r\n.global_fade_toggle {\r\n    background: rgba(0, 0, 0, 0.6);\r\n    display: block;\r\n}\r\n\r\n/*全局复杂select*/\r\n.global-cells{\r\n    margin-top: 10px;\r\n    position: relative;\r\n}\r\n.global-cells:before {\r\n    content: \" \";\r\n    position: absolute;\r\n    left: 0;\r\n    top: 0;\r\n    width: 100%;\r\n    height: 1px;\r\n    border-top: 1px solid #cdcdcd;\r\n    color: #cdcdcd;\r\n    -webkit-transform-origin: 0 0;\r\n    transform-origin: 0 0;\r\n    -webkit-transform: scaleY(0.5);\r\n    transform: scaleY(0.5);\r\n}\r\n.global-cells:after {\r\n    content: \" \";\r\n    position: absolute;\r\n    left: 0;\r\n    bottom: 0;\r\n    width: 100%;\r\n    height: 1px;\r\n    border-bottom: 1px solid #cdcdcd;\r\n    color: #cdcdcd;\r\n    -webkit-transform-origin: 0 100%;\r\n    transform-origin: 0 100%;\r\n    -webkit-transform: scaleY(0.5);\r\n    transform: scaleY(0.5);\r\n}\r\n.global-cell{\r\n    background-color: #fff;\r\n    padding:10px 15px;\r\n}\r\n.global-cell-title{\r\n    font-size: 16px;\r\n    font-weight: 900;\r\n    color:#7c7c7c;\r\n}\r\n.global-cell-li{\r\n    color:#7c7c7c;\r\n}\r\n.global-pop-select{\r\n    position: fixed;\r\n    top:0;\r\n    left:0;\r\n    background-color:#f3f3f3;\r\n    width: 100%;\r\n    height:100%;\r\n    z-index: 2;\r\n    overflow-y: auto\r\n}\r\n.global-goback{\r\n    position: fixed;\r\n    bottom:0;\r\n    left:0;\r\n    text-align: center;\r\n    line-height: 36px;\r\n    width: 100%;\r\n    background-color: #fff;\r\n    z-index: 1;\r\n}\r\n.global-goback:before {\r\n    content: \" \";\r\n    position: absolute;\r\n    left: 0;\r\n    top: 0;\r\n    width: 100%;\r\n    height: 1px;\r\n    border-top: 1px solid #cdcdcd;\r\n    color: #cdcdcd;\r\n    -webkit-transform-origin: 0 0;\r\n    transform-origin: 0 0;\r\n    -webkit-transform: scaleY(0.5);\r\n    transform: scaleY(0.5);\r\n}\r\n\r\n/* weui扩展 checkbox 加上 min 样式变小号*/\r\n.weui_switch.min{height:26px;width:46px;border-radius:14px;}\r\n.weui_switch.min:before,.weui_switch.min:after{height:24px;border-radius:100%;}\r\n.weui_switch.min:after{width:24px;}\r\n\r\n/*全局右下角半透明圆球*/\r\n.global-ball{position:fixed;bottom:10px;right:10px;height:50px;width:50px;background:white;border-radius:100%;box-shadow:1px 1px 4px #ccc;padding:0;display:-webkit-box;-webkit-box-align:center;-webkit-box-pack:center;}\r\n.global-ball:active{background:#f2f2f2;}\r\n/*全局loading*/\r\n.global-loading,.global-loading-top,.global-loading-bottom,.global-loading-hover{position:relative;}\r\n.global-loading-bottom{padding-bottom: 55px;}\r\n.global-loading:after,.global-loading-top:after{content:'\\52A0\\8F7D\\4E2D~';font-size:12px;line-height:140px;color:#888;text-align:center;vertical-align:bottom;display:block;background:url(" + __webpack_require__(309) + ") center no-repeat;top:0px;left:0;position:absolute;width:100%;height:80px;z-index:1000;}\r\n.global-loading-bottom:after{content:'\\52A0\\8F7D\\4E2D~';font-size:12px;line-height:140px;color:#888;text-align:center;vertical-align:bottom;display:block;background:url(" + __webpack_require__(309) + ") center no-repeat;bottom:0;left:0;position:absolute;width:100%;height:80px;z-index:1000;}\r\n.global-loading-hover:after{content:'\\52A0\\8F7D\\4E2D~';font-size:12px;line-height:140px;color:#888;text-align:center;vertical-align:bottom;display:block;background:url(" + __webpack_require__(309) + ") 50% 20px no-repeat;background-color:rgba(255,255,255,1);top:0px;left:0;position:absolute;width:100%;height:100%;min-height:80px;z-index:1000;}\r\n.global-no-data:after{content:'\\6682\\65E0\\6570\\636E';display:block;font-size:12px;color:#888;text-align:center;line-height:60px;}\r\n\r\n/*头部Tab*/\r\n.global-header-tab{display:-webkit-box;height:40px;line-height:40px;background:white;margin-bottom:10px;border-bottom:1px solid #ddd;}\r\n.global-header-tab>div{-webkit-box-flex:1;text-align:center;border-right:1px solid #ddd; width:25%;}\r\n.global-header-tab>div.active{background:#F2F2F2;border-bottom:4px solid #04BE02;}\r\n.global-header-tab>div:last-child{border-right:0;}\r\n\r\n.global-toast{position:fixed;bottom:50px;left:50%;-webkit-transform:translateX(-50%);z-index:10000;background:rgba(0,0,0,0.8);color:white;font-size:12px;padding:3px 8px;border-radius:3px;box-shadow:3px 3px 3px #333;-webkit-transition:0.4s;}\r\n.global-wechat-popup{padding:0 20px;width:70%;}\r\n.global-wechat-popup>div{line-height:50px;border-bottom:1px solid #ccc;}\r\n.global-wechat-popup>div:last-child{border-bottom:0;}\r\n.global-wechat-popup>div>span{padding:10px 15px;border-radius:20px;}\r\n.global-wechat-popup>div:active span{background:#ccc;}\r\n\r\n/*地区选择*/\r\n#chooseArea{width:100%; height:100%;position:fixed; left:0; top:0; overflow: hidden;z-index: 1;}\r\n#chooseArea .area-mark{width:100%;background:rgba(0,0,0,.5);height:100%;position:absolute; left:0; bottom:0; top:0;}\r\n#chooseArea .area-tab{width:80%; position:absolute; right:0; top:0; overflow:hidden; height:100%;}\r\n#chooseArea .area-tab-menu{width:100%; height:40px; line-height: 40px; font-size:14px; text-align: center; border-bottom:1px solid #ddd; background:#eee; display: -webkit-box;}\r\n#chooseArea .area-tab-menu li{-webkit-box-flex:1; box-sizing: border-box; cursor: pointer;}\r\n#chooseArea .area-tab-menu li.active{color:#0dc459; border-bottom:2px solid #0dc459;}\r\n#chooseArea .area-tab-con{width:100%; background:#fff; overflow:hidden; height:100%; position:relative;}\r\n#chooseArea .area-tab-con ul{width:100%; background:#fff; overflow:auto;overflow-x: hidden;padding-bottom: 40px}\r\n#chooseArea .area-tab-con ul li{height:40px; line-height: 40px; text-indent:20px; border-bottom:1px solid #e5e5e5; cursor: pointer;}\r\n#chooseArea .area-tab-con ul li.active{color:#0dc459;}\r\n#chooseArea .area-tab-con ul li:last-child{border:0 none;}\r\n#chooseArea .loadtip{position:absolute; width:100%; height:100%; left:0; top:0; background:rgba(0,0,0,.1); display:-webkit-box; -webkit-box-align: center; -webkit-box-pack:center;}\r\n#chooseArea .loadtip:after{content:'\\E613'; font-size:20px; width:20px; height:20px; line-height: 20px; font-family: 'wechat'; display:block; color:#555;-webkit-animation:loading 2s infinite linear;}\r\n\r\n.xa-webkit-box{display:-webkit-box;}\r\n.xa-webkit-box-vertical{display:-webkit-box;-webkit-box-orient:vertical;}\r\n.xa-flex{-webkit-box-flex:1;}\r\n.xa-txt-12{font-size:12px;}\r\n.xa-txt-13{font-size:13px;}\r\n.xa-txt-16{font-size:16px;}\r\n.xa-txt-bold{font-weight:bold;}\r\n.xa-txt-white{color:white;}\r\n.xa-txt-center{text-align:center;}\r\n.xa-line-40{line-height:40px;}\r\n.xa-line{line-height:24px;padding:8px 10px;}\r\n.xa-line-min{line-height:20px;padding:4px 10px;font-size:14px;}\r\n.xa-wekbit-center{display:-webkit-box;-webkit-box-align:center;-webkit-box-pack:center;}\r\n.xa-bg-white{background:white;}\r\n.xa-txt-666{color:#666;}\r\n.xa-txt-888{color:#888;}\r\n.xa-txt-right{text-align:right;}\r\n \r\n\r\n", ""]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "./images/loading_data.gif?v=3MpbiKb";
...
/***/ function(module, exports, __webpack_require__) {
	...
	// module
	exports.push([module.id, "\n@font-face {font-family: \"iconfont\";\n  src: url(" + __webpack_require__(312) + "); /* IE9*/\n  src: url(" + __webpack_require__(312) + "#iefix) format('embedded-opentype'), \n  url(" + __webpack_require__(313) + ") format('woff'), \n  url(" + __webpack_require__(314) + ") format('truetype'), \n  url(" + __webpack_require__(315) + "#iconfont) format('svg'); /* iOS 4.1- */\n}\n\n.iconfont {\n  font-family:\"iconfont\" !important;\n  font-size:16px;\n  font-style:normal;\n  -webkit-font-smoothing: antialiased;\n  -webkit-text-stroke-width: 0.2px;\n  -moz-osx-font-smoothing: grayscale;\n}\n.icon-shezhi:before { content: \"\\E603\"; }\n.icon-bianji:before { content: \"\\E646\"; }\n.icon-sousuo:before { content: \"\\E61A\"; }\n.icon-xiazai:before { content: \"\\E630\"; }\n.icon-rili:before { content: \"\\E63D\"; }\n.icon-dianhua:before { content: \"\\E62D\"; }\n.icon-daiban:before { content: \"\\E61D\"; }\n.icon-youjiantou:before { content: \"\\E635\"; }\n.icon-ditu:before { content: \"\\E605\"; }\n.icon-sousuo1:before { content: \"\\E625\"; }\n.icon-daiban1:before { content: \"\\E606\"; }\n.icon-fanhui:before { content: \"\\E604\"; }\n.icon-shangchuan:before { content: \"\\E631\"; }\n.icon-lishi:before { content: \"\\E617\"; }\n.icon-weizhi:before { content: \"\\E62C\"; }\n.icon-iconfontgouxuan:before { content: \"\\E618\"; }\n.icon-chacha:before { content: \"\\E626\"; }\n.icon-jixiao:before { content: \"\\E63E\"; }\n.icon-danxuankuang:before { content: \"\\E61B\"; }\n.icon-danxuankuang1:before { content: \"\\E61C\"; }\n.icon-zuobiao:before { content: \"\\E62A\"; }\n.icon-zanwujilu:before { content: \"\\E621\"; }\n.icon-wurenji:before { content: \"\\E607\"; }\n.icon-tongji:before { content: \"\\E608\"; }\n.icon-longtouhuasa:before { content: \"\\E645\"; }\n.icon-chacha1:before { content: \"\\E641\"; }\n.icon-huikuan:before { content: \"\\E609\"; }\n.icon-saomiao:before { content: \"\\E643\"; }\n.icon-xiangxia:before { content: \"\\E619\"; }\n.icon-xuanze:before { content: \"\\E628\"; }\n.icon-truck:before { content: \"\\E60A\"; }\n.icon-zhanghaotouxiang:before { content: \"\\E601\"; }\n.icon-xingxing:before { content: \"\\E623\"; }\n.icon-jinggao:before { content: \"\\E637\"; }\n.icon-xiugaimima:before { content: \"\\E600\"; }\n.icon-iconfontwode:before { content: \"\\E63F\"; }\n.icon-fanhui1:before { content: \"\\E624\"; }\n.icon-wode:before { content: \"\\E636\"; }\n.icon-dui:before { content: \"\\E642\"; }\n.icon-caogaoxiang:before { content: \"\\E60B\"; }\n.icon-baobiao:before { content: \"\\E60C\"; }\n.icon-shengpi:before { content: \"\\E60D\"; }\n.icon-jizhan:before { content: \"\\E633\"; }\n.icon-xuqiu:before { content: \"\\E60E\"; }\n.icon-daiban2:before { content: \"\\E61F\"; }\n.icon-tousu:before { content: \"\\E60F\"; }\n.icon-shenpi:before { content: \"\\E640\"; }\n.icon-richengjintian:before { content: \"\\E63C\"; }\n.icon-cuowutishi:before { content: \"\\E602\"; }\n.icon-dianchi:before { content: \"\\E639\"; }\n.icon-kytiandi:before { content: \"\\E610\"; }\n.icon-kehu:before { content: \"\\E611\"; }\n.icon-yiwancheng:before { content: \"\\E612\"; }\n.icon-jiaochacross80:before { content: \"\\E62B\"; }\n.icon-dui1:before { content: \"\\E644\"; }\n.icon-hetong:before { content: \"\\E613\"; }\n.icon-cubelifangti:before { content: \"\\E614\"; }\n.icon-tongji1:before { content: \"\\E634\"; }\n.icon-choose:before { content: \"\\E629\"; }\n.icon-shenpi03:before { content: \"\\E615\"; }\n.icon-gongzuo:before { content: \"\\E620\"; }\n.icon-dingwei:before { content: \"\\E632\"; }\n.icon-renwu:before { content: \"\\E616\"; }\n.icon-xiala:before { content: \"\\E627\"; }\n.icon-relitu:before { content: \"\\E63A\"; }\n.icon-xiangxia-copy:before { content: \"\\E61E\"; }\n.icon-xiangyou:before { content: \"\\E622\"; }\n.icon-zhanneixin:before { content: \"\\E62E\"; }\n.icon-xinfengdakai:before { content: \"\\E62F\"; }\n.icon-attention:before { content: \"\\E638\"; }\n.icon-zuojiantou:before { content: \"\\E63B\"; }\n", ""]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "./images/iconfont/iconfont.eot?v=1FTFSrC";

/***/ },
/* 313 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "./images/iconfont/iconfont.woff?v=1a8qkdK";

/***/ },
/* 314 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "./images/iconfont/iconfont.ttf?v=9R1hYE1";

/***/ },
/* 315 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "./images/iconfont/iconfont.svg?v=JgM3Q7F";
...
/***/ function(module, exports, __webpack_require__) {
	...
	Vue.component('toast', {
	  ...
	  filters: {
	    getClassName: function getClassName(params) {
...
/***/ function(module, exports) {
	...
	*/
	module.exports = {
	    "map": {
	        '/': {
	            component: Xa.requireModule("/master/master"),
	            subRoutes: {
	                'login': {
	                    component: Xa.requireModule("/login/login")
	                },
	                'main': {
	                    component: function () {
	                        return Xa.requireModule("/main/main");
	                    }()
	                }
	            }
	        }
