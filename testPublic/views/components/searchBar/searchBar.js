...
/***/ function(module, exports, __webpack_require__) {
	...
	var option = {
	    ...
	    watch: {
	        ...
	        'config.searchVal': function configSearchVal(val, oldVal) {
	            if (val != oldVal) {
	                this.$dispatch("searchBar-msg", {
	                    msg: "changeSearchVal",
	                    data: this.config.searchVal
	                });
	            }
	        },
	        'filterResult': {
	            handler: function handler(val, oldVal) {
	                ...
	                this.$dispatch("searchBar-msg", {
	                    msg: "changeFilterResult",
	                    data: this.filterResult
	                });
	            },
	            deep: true
	        },
	        "filterConfig.currItem": {
	            handler: function handler(val, oldVal) {
	                // debugger;
	                var strVal = JSON.stringify(val);
	                if (strVal.length == 2) {
	                    this.$dispatch("searchBar-msg", {
	                        msg: "customSearchEvent"
	                    });
	                }
	            },
	            deep: true
	    ...
	    computed: {
	        hasFilter: function hasFilter() {
	            return !Xa.Tools.isEmpty(this.filterResult);
	        }
	    },
	    methods: {
	        /**搜索 点击  “搜索”
	         * @param  {} type
	         */
	        'customSearchEvent': function customSearchEvent(type) {
	        ...
	        /**搜索 点击 "取消"
	         */
	        'searchCancelFn': function searchCancelFn() {
	            // console.log("searchCancelFn:" + JSON.stringify(arguments));
	            this.config.isFocusing = false;
	            this.searchVal = "";
	        },
	        'searchInputFn': function searchInputFn() {
	            this.config.isFocusing = true;
	        },
	        'searchClearFn': function searchClearFn() {
	        ...
	        "clickFilter": function clickFilter(item) {
	            ...
	            if (Xa.Tools.isEmpty(this.filterConfig.currItem)) {
	                //当前筛选为空对象 着是展开
	         ...
	         * @param  {} action
	         */
	        "clickItem": function clickItem(item, action) {
	            if (!!item) {
	                if (action == "select") {
	                    //单选的选择一次就结束了
	                ...
	                //item === null 时  说明点选了空白的遮罩区域 此时让展开的下拉的筛选区域隐藏
	                this.filterConfig.currItem = {};
	                this.currSelect = [];
	            }
	        }
	    ...
	    filters: {
	        "isEmptyObj": function isEmptyObj(params) {
	         ...
	         * @param  {} filterResult
	         */
	        "eventShow": function eventShow(cls, mmItem, type) {
	            // console.log(JSON.stringify(arguments));
	            if (type == 'filterItem') {
	        ...
	        "titleShow": function titleShow(def, item) {
